...
const assertions = [
  ...
  [home.includes('/styles.css?v=20260802-6') && home.includes('/script.js?v=20260802-6') && article.includes('/styles.css?v=20260802-6') && article.includes('/script.js?v=20260802-6'), 'Versioned static assets are missing'],
  [styles.includes('body[data-page="home"] {') && styles.includes('body[data-page="home"]::before') && styles.includes('--paper: #f4ecd9;') && script.includes("const lightThemeColor = '#f4ecd9';"), 'Unified homepage and editorial article palette is missing'],
  ...
  [script.includes("document.querySelector('.article-paper')") && script.includes('articleContent.getBoundingClientRect().bottom') && script.includes('readingProgress.style.width') && script.includes('window.requestAnimationFrame(updateReadingProgress)') && !script.includes('readingProgress.style.transform'), 'Article-bounded reading progress updates are missing'],
  ...
  [article.includes('data-article-style="editorial"'), 'Editorial article style should be fixed'],
  [!article.includes('class="article-style-toggle"'), 'Article style toggle should be removed'],
  [!script.includes('xycdev-blog-article-style') && !script.includes('applyArticleStyle') && !script.includes('articleStyleToggle'), 'Article style switching logic should be removed'],
